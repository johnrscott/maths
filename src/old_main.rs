...
impl Formula {
    ...
    fn from_quantifier(quantifier: Rc<Quantifier>) -> Self {
...
impl Quantifier {
    fn universal(variable: Rc<Variable>, formula: Rc<Formula>) -> Self {
	Self::Universal(variable, formula)
    }
    fn existent(variable: Rc<Variable>, formula: Rc<Formula>) -> Self {
...
fn main() {
    ...
    let p_and_q = Rc::new(Formula::BinaryConnective(Rc::new(BinaryConnective::and(p.clone(), q.clone()))));
    let p_or_q = Rc::new(Formula::BinaryConnective(Rc::new(BinaryConnective::or(p, q))));

    let for_all_x_p_and_q = Rc::new(Quantifier::universal(x.clone(), p_and_q.clone()))
    
    println!("{for_all_x_p_and_q}");
}
